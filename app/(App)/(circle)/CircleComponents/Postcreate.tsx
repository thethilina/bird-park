...
function PostCreate({ circleId , postCreateOpen }: PostCreateProps) {
  ...
  const analyzeArtEmotion = async (postId: string, file: File) => {
    try {
      ...
      const top3Emotions = data.emotional_profile.themes.map(
        (emotion: any) => ({ emotion: emotion.name, score: emotion.weight })
      );

      await fetch(`/api/post/${postId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ top3Emotions }),
      });
    } catch (err) {
      console.error(err);
    }
  ...
  const analyzePoemEmotion = async (postId: string, poemText: string) => {
    try {
      ...
      await fetch(`/api/post/${postId}`, {
        ...
        body: JSON.stringify({
          top3Emotions,
          emotionAnalysis: { status: "completed", completedAt: new Date() },
        }),
      });
    } catch (err) {
      console.error(err);
      await fetch(`/api/post/${postId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ emotionAnalysis: { status: "failed" } }),
      });
    }
  };

    ...
    <div className="pt-5 px-3 md:px-5 pb-20">
      ...
      <div className="mb-8">
        ...
        <p className="mt-2 text-sm text-neutral-500 dark:text-neutral-400">
          Creators you are observing to follow their artistic updates.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="max-w-md mx-auto my-8 flex flex-col items-center justify-center py-12 px-4 text-center border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-950/20 rounded-xl">
          <div className="text-red-500 dark:text-red-400 font-medium text-lg mb-2">Failed to load observing users</div>
        ...
        <div className="flex flex-col items-center justify-center py-16 px-4 text-center border border-dashed border-neutral-300 dark:border-neutral-700 rounded-xl">
          <p className="text-neutral-500 dark:text-neutral-400">You are not observing any users yet. Check suggestions to observe users!</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {users.map((user: any) => (
            <ObservingCard 
              observing={user} 
              key={user.requestId || user._id} 
              onRemove={(id: string) => {
                setUsers((prev: any[]) => prev.filter((r: any) => r._id !== id));
              }} 
            />
          ))}
        </div>

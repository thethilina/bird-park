...
function CircleSideBar() {
  ...
  return (
    <>
      <nav className='hidden lg:block w-72 fixed top-20 h-screen pr-5 bg-(--color-background) dark:bg-[#06060B] flex-col items-center py-5 gap-y-5 z-20'>
        <div className='sticky top-30 space-y-6 px-4'>
          {/* searchbar */}
          <div className="hidden lg:block relative">
            ...
            <input
              ...
              type="text"
              className="font-sans bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-full py-2 px-4 pr-10 w-full text-sm outline-none dark:text-white dark:placeholder-gray-400 focus:border-white/30 transition-colors"
            />
          </div>

          {/* middle buttons */}
          <div className="flex flex-col text-sm font-medium gap-y-1">
          ...
          <div className="flex flex-col gap-y-3 pt-4">
            <div className='flex items-center gap-x-3 px-2'>
              ...
              <h1 className='text-sm font-medium text-gray-500 dark:text-gray-400'>Circles you are in</h1>
            </div>
              ...
              <div className="flex flex-col gap-1 max-h-[50vh] overflow-y-auto pr-1">
                {filtered.map((c) => (
                  <Link
                    key={c._id}
                    href={`/Circle/${c._id}`}
                    className="flex items-center gap-3 p-2 hover:bg-black/5 dark:hover:bg-white/5 rounded-xl text-sm transition-colors dark:text-gray-200"
                  >
                    <div className="w-8 h-8 rounded-lg overflow-hidden bg-gray-200 dark:bg-gray-800 flex-shrink-0 flex items-center justify-center text-xs font-bold dark:text-white text-gray-600 shadow-sm">
                      {c.icon ? (
